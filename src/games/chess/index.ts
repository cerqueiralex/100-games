...
export const chessDefinition: GameDefinition = {
  ...
  mastery,
  scoringNote:
    'Full chess against the robot — you play White. Castling, en passant, promotion, check and checkmate all work exactly as over the board, and stalemate, threefold repetition, the fifty-move rule and dead positions are called as draws. Drag a piece (or tap-then-tap) to move; the last move, checks and your legal squares are highlighted, captures collect in the trays and every move lands in the log. Checkmate the robot to win: 400 × difficulty (1–5) plus 20 per pawn of material lead. A draw pays 60 × difficulty. The robot is the Stockfish engine held to a real strength per tier: about 300 Elo on easy (a weighted lottery over the legal moves — no search at all), 750 on medium (a shallow search that picks loosely among its best few lines and answers one move in seven like a beginner), 1200 on hard (a deeper search, still picking loosely, never a beginner move), 1600 on pro (its own strength limiter) and full strength on extreme — each tier proven stronger than the one below by matches. The engine (7 MB) downloads once, the first time a medium-or-harder game starts.',
  assistFeatures: [
    {
      id: 'hint',
      name: 'Hint',
