...
- `npm run validate` — integrity checks for game content: rebuilds every hand-authored crossword (intersections, accidental adjacent words), test-generates sudoku, Cryptogram, Logic Puzzles and Color Connect puzzles, and checks the Word Wheel and Cryptogram content banks. **Run this after any edit to `src/games/crossword/logic/puzzles.ts`, `src/games/word-wheel/logic/levels.ts`, or `src/games/cryptogram/logic/words.ts`.**
