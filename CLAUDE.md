...
- Surfaces are **flat frosted glass with NO gradients**, defined once in `design/effects.css` and bound both to `.fx-card` and to every shared card-component class listed there — component CSS never declares its own card background/border (this is what keeps Settings etc. in sync with redesigns). Depth comes from the drop shadow only: no highlight/specular edges, no inset highlights, no bevels, no 3D transforms (the old `Tilt.tsx` tilt/glow component was removed deliberately — don't reintroduce it). Deliberate flat opt-outs (home search bar, in-game info strip) use plain `var(--surface)` + `var(--border)` with a marking comment.
